...
Future<http.Response> getData() async {
  // const lokal = "192.168.137.5";
  final response =
      await http.get(Uri.parse("http://192.168.137.5:8082/api/users/getAll"));
  return response;
}
...
Future<http.Response> postData(Map<String, String> data) async {
  final response =
      await http.post(Uri.parse("http://192.168.137.5:8082/api/users/insert"),
...
Future<http.Response> updateData(id) async {
  ...
  final response = await http.put(
      Uri.parse("http://192.168.137.5:8082/api/users/update/${id}"),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(data));
...
Future<http.Response> deleteData(int id) async {
  final response = await http.delete(
      Uri.parse("http://192.168.137.5:8082/api/users/delete/${id}"),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      });
  print(response.statusCode);
